...
public class PengeluaranForm extends JPanel {
    ...
    private final DefaultTableModel model = new DefaultTableModel(
            new String[]{"ID", "Tanggal", "Kategori", "Nominal", "Keterangan"}, 0) {
    ...
    public PengeluaranForm() {
        ...
        JPanel p = UIUtils.page("Pengeluaran");
    ...
    private void load() {
        model.setRowCount(0);
        double sum = 0;
        for (Pengeluaran x : c.getByPeriode(
                LocalDate.parse(dari.getText()),
                LocalDate.parse(sampai.getText()))) {
            sum += x.getNominal();
            model.addRow(new Object[]{
                x.getId(), x.getTanggal(), x.getNamaKategori(),
                UIUtils.rupiah(x.getNominal()), x.getKeterangan()
            });
        }
    ...
    private void showDialog() {
        ...
        simpan.addActionListener(e -> {
            try {
                ...
                Pengeluaran val = new Pengeluaran(
